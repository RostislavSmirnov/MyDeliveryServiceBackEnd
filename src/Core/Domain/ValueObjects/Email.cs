...
public sealed record Email
{
    ...
    public static Email Create(string? email)
    {
        ...
        if (trimmed.Length > 254)
            throw new ArgumentException("Email слишком длинный (максимум 254 символа).", nameof(email));

        if (!EmailRegex.IsMatch(trimmed))
            throw new ArgumentException("Некорректный формат email-адреса.", nameof(email));

        if (!trimmed.Contains('@') || trimmed.Split('@')[1].Split('.').Length < 2)
            throw new ArgumentException("Некорректный формат email-адреса.", nameof(email));
