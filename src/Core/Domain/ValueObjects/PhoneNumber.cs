...
public sealed record PhoneNumber
{
    ...
    public static PhoneNumber Create(string? phoneNumber)
    {
        ...
        if (string.IsNullOrEmpty(cleaned))
            throw new ArgumentException("Номер телефона не может быть пустым после очистки.", nameof(phoneNumber));

        // Проверка длины (E.164 рекомендует максимум 15 цифр)
        if (cleaned.Length > 15)
            throw new ArgumentException("Номер телефона слишком длинный (максимум 15 цифр).", nameof(phoneNumber));

        if (!PhoneRegex.IsMatch(cleaned))
            throw new ArgumentException("Некорректный формат номера телефона. Используйте международный формат (например: +79161234567).", nameof(phoneNumber));

        // Дополнительная проверка: должен начинаться с + или цифры
        if (!cleaned.StartsWith("+") && !char.IsDigit(cleaned[0]))
            throw new ArgumentException("Номер телефона должен начинаться с '+' или цифры.", nameof(phoneNumber));
